...
- **Controller state (from `experiments/exp_062_mutual_best_edge_association/experiment.json`):** READY
- **Phase:** EXP062_ADMISSION_PASS_AND_SMOKE_PASS_AWAITING_LAUNCH_AUTHORIZATION
...
- **Next action:** exp_062 has a fresh Codex ADMISSION PASS (2026-09-24) and a PASSED snapshot smoke. Nothing remote has run. NEXT, and each needs explicit user authorization: reserve 3.0 GPU h, then launch k1 (control) and verify it reproduces the parent submission SHA d34533806b3153ddd4f33f3bbc1dea70af2d5406bb1ea48e42c135a97c213f60 -- if it does not, STOP, the insertion is not inert. Then launch k2 (candidate). Then, only if k2 differs from the control, ONE authorized LB submission of k2 against the three-per-New-York-day project cap. Decision rule vs 0.947: >=0.948 adopt; ==0.947 CLOSE the probe (no beta escalation); <=0.946 revert. Designated successor either way: density-adaptive relinking, which needs its own proposal.

_Generated from `STATE.json` (updated 2026-09-24T05:00:00+00:00). Do not hand-edit this block; edit `STATE.json` and rerun `python scripts/render_checkpoint.py`. Full authorization detail lives once in `GOAL.md`._
<!-- END AUTO-CHECKPOINT -->
