...
EXPERIMENT = "repro_048_public_0946_exact_copy"
PROTOCOL = "public_0946_submission_integrity_lb_probe_v1"
...
def collect(root: Path) -> dict:
    ...
    if record["state"] == "REMOTE_FAILED" and record.get("remote", {}).get("status") == "COMPLETE":
        # A sandbox-blocked output download is recoverable without rerunning the
        # already-complete remote kernel. Restore the state machine explicitly.
        transition(root, record, "READY", note="Recovering completed remote output after local download failure")
        record = load_record(root, EXPERIMENT)
        transition(root, record, "SUBMITTED", note="Remote kernel was already complete; no relaunch")
        record = load_record(root, EXPERIMENT)
    if record.get("remote", {}).get("status") != "COMPLETE":
    ...
    try:
        ...
        payload = {"schema_version": 1, "experiment_id": EXPERIMENT,
                   "primary_metric": 1.0, "runtime_seconds": runtime,
                   "reproducible": False,
                   "validation": {"protocol": PROTOCOL, "sample_count": 4,
                                  "warning": "Binary execution and graph-integrity check; quality is decided only by the external Public LB."},
