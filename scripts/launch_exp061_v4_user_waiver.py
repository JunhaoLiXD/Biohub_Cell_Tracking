"""One-time tracked Kaggle launch for the user's explicit v4 review waiver.

This does not claim a Codex PASS or change the generic controller gates.
"""
...
ROOT = PROJECT_ROOT
EXPERIMENT_ID = "exp_061_zon_lb_submission_repair_v4"
EXPECTED_SHA256 = "39eba1b92c79739b324009a274336672fa098e8f11c4edf1e16c16da6be08290"
VALIDATOR = ROOT / "scripts" / "validate_exp061_zon_deployment_v4.py"


def main() -> int:
    record = load_record(ROOT, EXPERIMENT_ID)
    if record["state"] != "PROPOSED" or record.get("remote"):
        raise ControllerError("One-time v4 launch requires an untouched PROPOSED record and no remote run")
    verify_snapshot(ROOT, record)
    source = ROOT / record["snapshot_source"]
    if sha256_file(source) != EXPECTED_SHA256:
        raise ControllerError("v4 notebook hash changed")
    _, config = load_config(ROOT / record["snapshot_config"], ROOT)
    settings = resolve_kernel_settings(config)
    if settings["kernel_id"] != "lingxd/biohub-exp061-zon-deploy-v4":
    ...
    if check.returncode:
        raise ControllerError("v4 immutable snapshot validation failed; see user-waiver-smoke.json")

    record["review"] = {
        "required": True,
        "provider": "codex",
        "status": "WAIVED_BY_USER_FOR_ONE_V4_KERNEL_RUN",
        "verdict": "NO_PASS",
        "reason": "User explicitly requested Kaggle run after being told formal admission remained BLOCK.",
        "at_utc": utc_now(),
    }
    record["smoke_test"] = {
        "status": "PASSED",
