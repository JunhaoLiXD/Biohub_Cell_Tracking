"""Build the Phase-1 factorized Family-A policy notebook."""
...
TARGET = ROOT / ".private" / "current" / "phase1_factorized_policy.ipynb"
SOURCES = (
    ...
    "cells": [
        {
            ...
            "metadata": {},
            "source": (
                "# Phase 1 factorized Family-A policy gate\n\n"
                "This CPU-only diagnostic replaces the direct ultra-rare action classifier with "
                "a factorized score: endpoint validity probability times conditional division-edge "
                "probability. The feature set, action family, per-video cap, frozen scorer, and "
                "training-specimen-only threshold calibration remain unchanged from diag_028.\n"
        ...
        *[code_cell(without_main(path), f"embedded-{index}") for index, path in enumerate(SOURCES)],
        code_cell(
            "payload = run_factorized_policy_gate()\n"
        ...
        code_cell(
            ...
            "assert saved_metrics['experiment_id'] == 'diag_029_train16_factorized_family_a_policy'\n"
            "assert isinstance(saved_metrics['metrics']['factorized_policy_gate_passed'], bool)\n"
