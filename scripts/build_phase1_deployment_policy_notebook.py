"""Build the Phase-1 deployment-complete Family-A policy notebook."""
...
TARGET = ROOT / ".private" / "current" / "phase1_deployment_policy.ipynb"
SOURCES = (
    ...
    "cells": [
        {
            ...
            "metadata": {},
            "source": (
                "# Phase 1 deployment-complete Family-A policy gate\n\n"
                "This CPU-only diagnostic keeps the diag_027 model and action policy fixed while "
                "adding actual deployable Family-A candidates to the negative training distribution. "
                "Ground-truth matching is used only for labels and post-selection diagnostics, never "
                "as a model feature. Thresholds remain training-specimen-only.\n"
        ...
        *[code_cell(without_main(path), f"embedded-{index}") for index, path in enumerate(SOURCES)],
        code_cell(
            "payload = run_deployment_policy_gate()\n"
        ...
        code_cell(
            ...
            "assert saved_metrics['experiment_id'] == 'diag_028_train16_deployment_complete_family_a_policy'\n"
            "assert isinstance(saved_metrics['metrics']['deployment_complete_policy_gate_passed'], bool)\n"
