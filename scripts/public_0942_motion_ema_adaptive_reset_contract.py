# Embedded final contract for the label-free adaptive EMA reset experiment.
...
_VAL039_VIDEO_ADJUSTED_EDGE = {
    "44b6_1d530831": 0.7969198174184619, "44b6_2a2eff9f": 0.89235322899701,
    "44b6_551a5dba": 0.8293364900661753, "44b6_7a302da0": 0.9631748927747178,
    "44b6_7e557709": 0.9697278692084568, "44b6_aaf8b0ea": 1.0069255657234752,
    "44b6_c15fded2": 0.828546217819058, "44b6_c50204e0": 0.8120402474453199,
    "6bba_0c7fa718": 0.8003078098372215, "6bba_283bf9f1": 0.9341409661473707,
    "6bba_337b1b3a": 0.9401114627658469, "6bba_372c8cb8": 1.0032753214280064,
    "6bba_55c70843": 0.7122589938896665, "6bba_80d12824": 0.894819771976011,
    "6bba_ef7b4f7e": 0.9788588208381466, "6bba_fe670320": 0.9750245558283982,
}
...
_adaptive_execution = {
    specimen: {
        ...
        "reset_updates": int(sum(row.get("motion_relink_adaptive_reset_updates", 0)
            for row in validator_sample_rows if row["stem"].startswith(specimen + "_"))),
        "base_updates": int(sum(row.get("motion_relink_adaptive_base_updates", 0)
            for row in validator_sample_rows if row["stem"].startswith(specimen + "_"))),
        "initial_updates": int(sum(row.get("motion_relink_adaptive_initial_updates", 0)
            for row in validator_sample_rows if row["stem"].startswith(specimen + "_"))),
...
_contract_checks = {
    **_inference_checks,
    "experiment_id_injected": _EXPERIMENT_ID == "exp_043_public_0942_motion_ema_adaptive_reset",
    ...
    "adaptive_reset_exercised_both_specimens": all(
        values["reset_updates"] > 0 for values in _adaptive_execution.values()),
    "adaptive_base_exercised_both_specimens": all(
        values["base_updates"] > 0 for values in _adaptive_execution.values()),
}
_research_gates = {
...
_metrics = {
    ...
    "validation": {
        ...
        "warning": "Frozen models saw training videos; this is an optimistic paired adaptive-policy test.",
    },
    "specimen_metrics": _specimens,
    "metrics": {
        "validation_contract_passed": all(_contract_checks.values()),
        "adaptive_ema_candidate_gate_passed": all(_contract_checks.values()) and all(_research_gates.values()),
        "checks": _contract_checks, "research_gates": _research_gates,
        ...
        "video_response_vs_val039": {
            ...
            "worst_video": _worst_video, "worst_delta": _video_deltas_vs_val039[_worst_video],
        },
        "motion_relink_adaptive_execution": _adaptive_execution,
