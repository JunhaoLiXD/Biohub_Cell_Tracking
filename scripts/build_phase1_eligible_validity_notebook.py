"""Build the Phase-1 eligible-conditioned validity policy notebook."""
...
TARGET = ROOT / ".private" / "current" / "phase1_eligible_validity_policy.ipynb"
SOURCES = (
    ...
    "cells": [
        {
            ...
            "metadata": {},
            "source": (
                "# Phase 1 eligible-conditioned validity policy gate\n\n"
                "This CPU-only diagnostic changes only the validity-model training distribution "
                "from a mixed matched-or-eligible pool to actual deployable Family-A candidates. "
                "The conditional division model, features, action family, frozen scorer, per-video "
                "cap, and training-specimen-only threshold calibration remain fixed.\n"
        ...
        *[code_cell(without_main(path), f"embedded-{index}") for index, path in enumerate(SOURCES)],
        code_cell(
            "payload = run_eligible_validity_policy_gate()\n"
        ...
        code_cell(
            ...
            "assert saved_metrics['experiment_id'] == 'diag_030_train16_eligible_validity_factorized_policy'\n"
            "assert isinstance(saved_metrics['metrics']['eligible_validity_policy_gate_passed'], bool)\n"
