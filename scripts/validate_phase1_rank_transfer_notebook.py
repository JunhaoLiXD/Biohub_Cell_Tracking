...
def main() -> int:
    ...
    required = (
        ...
        '"transferred_action_count"',
        '"44b6_to_6bba"',
        '"6bba_to_44b6"',
        '"selected_true_division_edges"',
        '"rank_transfer_policy_gate_passed"',
        '"gt_matching_is_not_a_model_feature"',
        "/kaggle/working/metrics.json",
    )
    missing = [item for item in required if item not in source]
    if missing:
        raise ControllerError(f"Rank-transfer notebook is missing required code: {missing}")
    summary["cross_specimen_directions"] = 2
    summary["rank_count_transfer"] = True
