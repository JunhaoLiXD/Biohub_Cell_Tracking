"""Build the Phase-1 structural rank-transfer policy notebook."""
...
TARGET = ROOT / ".private" / "current" / "phase1_rank_transfer_policy.ipynb"
SOURCES = (
    ...
    "cells": [
        {
            ...
            "metadata": {},
            "source": (
                "# Phase 1 structural rank-transfer policy gate\n\n"
                "This CPU-only diagnostic keeps the diag_031 structural factorized scorer fixed. "
                "It replaces absolute-probability transfer with the training-optimal global action "
                "count, selecting the same number of highest-ranked held-out video actions.\n"
        ...
        *[code_cell(without_main(path), f"embedded-{index}") for index, path in enumerate(SOURCES)],
        code_cell(
            "payload = run_rank_transfer_policy_gate()\n"
        ...
        code_cell(
            ...
            "assert saved_metrics['experiment_id'] == 'diag_032_train16_structural_rank_transfer_policy'\n"
            "assert isinstance(saved_metrics['metrics']['rank_transfer_policy_gate_passed'], bool)\n"
