"""Build the Phase-1 node-validity rank-policy notebook."""
...
TARGET = ROOT / ".private" / "current" / "phase1_node_validity_policy.ipynb"
SOURCES = (
    ...
    ROOT / "scripts" / "run_phase1_rank_transfer_policy.py",
    ROOT / "scripts" / "run_phase1_node_validity_policy.py",
)
    ...
    "cells": [
        {
            ...
            "metadata": {},
            "source": (
                "# Phase 1 node-validity rank-policy gate\n\n"
                "This CPU-only diagnostic replaces sparse pair-validity learning with node-level "
                "matched-versus-unmatched learning over every frozen prediction node. The division "
                "model, structural candidate features, Family-A constraints, rank-count transfer, "
                "and frozen scorer remain unchanged.\n"
        ...
        *[code_cell(without_main(path), f"embedded-{index}") for index, path in enumerate(SOURCES)],
        code_cell(
            "payload = run_node_validity_policy_gate()\n"
        ...
        code_cell(
            ...
            "assert saved_metrics['experiment_id'] == 'diag_033_train16_node_validity_rank_policy'\n"
            "assert isinstance(saved_metrics['metrics']['node_validity_policy_gate_passed'], bool)\n"
            "assert saved_metrics['metrics']['feature_contract_valid'] is True\n"
            "assert saved_metrics['metrics']['all_features_available_at_inference'] is True\n"
