...
EXPERIMENT_ID = "diag_016_train16_candidate_constrained_oracle_guarded"
...
def _load_relevant_candidates(
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line in handle:
            ...
            total_rows += 1
            if pair[0] not in valid_node_ids or pair[1] not in valid_node_ids:
                invalid_endpoint_rows += 1
            if pair in wanted:
                found[pair] = row
    return found, total_rows, invalid_endpoint_rows
...
def run(search_root: Path = Path("/kaggle/input"), output_path: Path = Path("/kaggle/working/metrics.json")) -> dict[str, Any]:
    ...
    for name in validation_names:
        ...
        candidates, candidate_row_count, invalid_endpoint_rows = _load_relevant_candidates(
            candidate_path,
            wanted,
            set(nodes),
        )
        ...
        if invalid_endpoint_rows:
            raise RuntimeError(
                f"Candidate/prediction node-id mismatch for {name}: "
                f"{invalid_endpoint_rows}/{candidate_row_count} rows have unknown endpoints"
            )
        per_video[name] = {
    ...
    payload = {
        ...
        "metrics": {
            ...
            "candidate_rows_by_video": {
                name: {
                    "total": video["candidate_row_count"],
