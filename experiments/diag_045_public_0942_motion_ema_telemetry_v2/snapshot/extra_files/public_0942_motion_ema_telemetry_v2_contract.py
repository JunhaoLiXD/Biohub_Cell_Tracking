# Embedded final contract for the corrected telemetry-only fixed-alpha-0.4 diagnostic.
...
_telemetry_complete = (
    ...
    and all(_test_row_contract(values) for values in _validation_telemetry.values())
)
...
_behavior_checks = {
    "aggregate_primary_exact": _telemetry_close(
        _summary["proxy_score"], TELEMETRY_V2_EXPECTED["aggregate"]["primary_metric"]),
    "aggregate_adjusted_edge_exact": _telemetry_close(
        _summary["adjusted_edge_jaccard"], TELEMETRY_V2_EXPECTED["aggregate"]["adjusted_edge_jaccard"]),
    "aggregate_division_exact": _telemetry_close(
        _summary["division_jaccard"], TELEMETRY_V2_EXPECTED["aggregate"]["division_jaccard"]),
    "specimens_exact": all(
        _telemetry_close(_specimens[s][key], TELEMETRY_V2_EXPECTED["specimens"][s][key])
        for s in _specimens
        for key in ("primary_metric", "adjusted_edge_jaccard", "division_jaccard")
    ),
    "videos_exact": set(_video_metrics) == set(TELEMETRY_V2_EXPECTED["videos"]) and all(
        all(_telemetry_close(_video_metrics[stem][key], expected)
            for key, expected in values.items())
        for stem, values in TELEMETRY_V2_EXPECTED["videos"].items()
    ),
    "division_counts_exact": {
        key: int(_overall[key]) for key in ("div_tp", "div_fp", "div_fn")
    } == TELEMETRY_V2_EXPECTED["division_counts"],
    "ema_execution_exact": _ema_execution == TELEMETRY_V2_EXPECTED["ema_execution"],
}
_contract_checks = {
    **_inference_checks,
    **_behavior_checks,
    "experiment_id_injected": _EXPERIMENT_ID == "diag_045_public_0942_motion_ema_telemetry_v2",
    "frozen_samples_and_order": all(
        _specimens[s]["samples"] == FROZEN_SAMPLES[s] for s in FROZEN_SAMPLES),
    "frozen_strata": all(
        set(row["stem"] for row in validator_sample_rows
            if row["stem"].startswith(s + "_") and division_flags[row["stem"]])
        == set(FROZEN_POSITIVES[s])
    ...
    "candidate_submission_unchanged_after_validation": _sha256_file(SUBMISSION_PATH)
        == TELEMETRY_V2_EXPECTED["submission_sha256"],
...
_metrics = {
    ...
    "baseline_primary_metric": TELEMETRY_V2_EXPECTED["aggregate"]["primary_metric"],
