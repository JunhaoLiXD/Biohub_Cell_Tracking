"""Validate exp061 zon-only v3 source parity, execution path, and fail-closed publication."""
...
from build_exp061_zon_submission_repair_v3 import BASE, MODULE, OUT, FINAL, IDENTITY_LINE, WATCHDOG_NEW, WATCHDOG_OLD
...
def assert_source_contract(path):
    ...
    assert nb["cells"][:2] == base["cells"][:2]
    ...
    for i, cell in enumerate(nb["cells"]):
        if cell["cell_type"] == "code":
            compile("".join(cell["source"]), f"v3_cell_{i}", "exec")
    tree = ast.parse(module)
    calls = [n for n in ast.walk(tree) if isinstance(n, ast.Call)]
    assert not any(isinstance(n.func, ast.Name) and n.func.id == "run_exp061_deepcenter_tta" for n in calls)
    ...
    assert "submission_parent_current.csv" not in module
    receipt = json.loads((OUT.parent / "build-receipt.json").read_text(encoding="utf-8"))
    assert receipt["base_sha256"] == hashlib.sha256(BASE.read_bytes()).hexdigest()
...
def load_deployment_module():
    return load_path("exp061_zon_deployment_v3", MODULE)
...
def run_mock(work, *, fourfold=False, no_veto=False, missing_score=False, omit_dataset=False):
    ...
    g["EXPERIMENT_ID"] = "exp_061_zon_lb_submission_repair_v3"
    if fourfold:
...
def validate(path):
    ...
    print("PASS: immutable parent cells and zon-only executable AST")
    ...
    print("PASS: final publication positives/negatives; parent fallback deleted before zon failure")
